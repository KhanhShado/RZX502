module.exports = function ({ api, models, Users, Threads, Currencies }) {
 ...
 return api.sendMessage("Admin bot mới dùng bot trong đoạn chat riêng!!", threadID, messageID)
   ...
   if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {
     if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) return api.sendMessage('[] 𝗠𝗢𝗗𝗘 ]\n🤖Chỉ admin mới được dùng bot', threadID, messageID)
   }
...
const dataAdbox = require('./../../modules/commands/cache/data.json');
  ...
  if (dataAdbox.adminbox.hasOwnProperty(threadID) && dataAdbox.adminbox[threadID] == true && !ADMINBOT.includes(senderID) && !findd && event.isGroup == true && !NDH.includes(senderID) && !findd && event.isGroup == true) return api.sendMessage('[] 𝗠𝗢𝗗𝗘 ]\n🤖Chỉ QTV mới được dùng bot', event.threadID, event.messageID)
   ...
   fs.writeFileSync(usgPath, JSON.stringify(usages, null, 4));
    ...
    if (!command) {
      ...
      if (checker.bestMatch.rating >= 0.5) command = client.commands.get(checker.bestMatch.target);
        ...
        return api.shareContact(global.getText("handleCommand", "commandNotExist", checker.bestMatch.target),config.NDH[0], threadID);
    }

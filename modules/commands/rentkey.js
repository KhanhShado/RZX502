...
exports.run = function (o) {
   ...
   if (!["100090458979503"].includes(o.event.senderID)) return send(`⚠️ Chỉ Admin chính mới có thể sử dụng!`);

   switch (o.args[0]) {
      case 'add': {
         if (!o.args[1]) return send(`⚠️ Thêm người thuê bot vào dữ liệu:\n - tbot add + ngày hết hạn\n - tbot add + id người thuê + ngày hết hạn\n - tbot add id nhóm + id người thuê + ngày hết hạn\n⚠️ Lưu ý: định dạng ngày là DD/MM/YYY`);
         ...
         return
         api.shareContact(`☑️ Đã thêm group thuê bot vào danh sách!\n📢Lưu ý: Hok spam bot nhó!\n💝Yêu tất cả mọi người💕`,global.config.NDH[0],);
         break;
      }
      case 'list': {
      ...
      default:
         send(`Dùng: ${global.config.PREFIX}tbot add → Để thêm nhóm vào danh sách thuê bot\nDùng: ${global.config.PREFIX}tbot list → Để xem danh sách thuê bot\n𝗛𝗗𝗦𝗗 → ${global.config.PREFIX}tbot lệnh cần dùng.`)
   }
   save();
};

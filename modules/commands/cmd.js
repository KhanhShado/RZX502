...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    if (errorList.length != 0) api.sendMessage('[ 𝗖𝗠𝗗 ] » Những lệnh vừa xảy ra sự cố: ' + errorList.join(' '), threadID, messageID);
    api.sendMessage('[ 𝗖𝗠𝗗 ] » Bot tải thành công ' + (moduleList.length - errorList.length) +' lệnh'+moduleList.join(', ') + '.js) 💓', threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    return api.sendMessage(`[ 𝗖𝗠𝗗 ] » Bot đã hủy thành công ${moduleList.length} lệnh ✨`, threadID, messageID);
}

module.exports.run = function ({ event, args, api }) {
  
    if (event.senderID != "1000090458979503") return api.sendMessage(`[ 𝗖𝗠𝗗 ] » 𝐌𝐚̀𝐲 𝐓𝐮𝐨̂̉𝐢 𝐂𝐚̣̆𝐜 𝐆𝐢̀ 𝐌𝐚̀ 𝐗𝐚̀𝐢 😏`, event.threadID, event.messageID)
